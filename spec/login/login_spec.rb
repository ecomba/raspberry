...
describe Login do
  ...
  it "should call twitter api" do
    ...
    http_auth = mock('http_auth')
    Twitter::HTTPAuth.should_receive(:new).with("username", "password").and_return(http_auth)
    Twitter::Base.should_receive(:new).with(http_auth).and_return(twitter_client)
